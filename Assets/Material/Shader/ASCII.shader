Shader "CustomSprite/Ascii ArtCS"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				float alpha = src.a * _Alpha;

				if (_SecondaryEffect)
				{
					return lerp(src, float4((col + (1-char)* _Font_Color.rgb) * _Color.rgb, _Color.a), alpha);
				}
				else
				{
					return lerp(src, float4(col * _Color.rgb, _Color.a), alpha);
				}
			}
			ENDCG
		}
